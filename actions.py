...
import random
import traceback
from utils import generate_random_email, phantom_confirm_tx, retry, switch_to_page_by_title, logger
from config import *
...
@retry(MAX_RETRY)
async def register_or_login_to_solayer(id, context, page):
    ...
    with open('ref_codes.txt', 'a') as file:
        for element in invite_code_elements[:3]:
            style = str(await element.get_attribute('style'))
            if 'line-through' not in style:
                invite_code = await element.text_content()
                invite_code = invite_code.strip()
                if invite_code not in existing_codes:
                    file.write('\n' + invite_code)
                    logger.info(f"{id} | New code added: {invite_code}")
                    existing_codes.add(invite_code)
                else:
                    logger.info(f"{id} | Duplicate code skipped: {invite_code}")
...
@retry(MAX_RETRY)
async def claim_episode_1(id, context, page):
    logger.info(f"{id} | START Claim episode 1")

    await page.goto('https://app.solayer.org/valley')

    await asyncio.sleep(random.uniform(1, 2))
    try:
        await page.click('a[routerlink="/episode1"]', timeout=10000)
    except:
        logger.error(f"{id} | Not eligible episode 1")
        return
    
    try:
        await page.wait_for_selector("div.gotoapp", timeout=10000)
        logger.success(f"{id} | Wallet already claimed episode 1")
    except:
        await page.wait_for_selector('svg.claim-btn', timeout=10000)

        claim_buttons = await page.query_selector_all('svg.claim-btn')

        for button in claim_buttons:
            await asyncio.sleep(random.uniform(2, 3))
            await button.click()
            
        await page.wait_for_selector("div.gotoapp")
        logger.success(f"{id} | Wallet has been claimed episode 1")
